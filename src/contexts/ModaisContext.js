...
function ModaisProvider({ children }) {
  ...
  const openModalSearchProduct = () =>
    dispatch({ type: "OPEN_MODAL_SEARCH_PRODUCT" });
  const closeModalSearchProduct = () =>
    dispatch({ type: "CLOSE_MODAL_SEARCH_PRODUCT" });

  const openModalSearchOrder = () =>
    dispatch({ type: "OPEN_MODAL_SEARCH_ORDER" });
  const closeModalSearchOrder = () =>
    dispatch({ type: "CLOSE_MODAL_SEARCH_ORDER" });

  const openModalWeightProduct = () =>
    dispatch({ type: "OPEN_MODAL_WEIGHT_PRODUCT" });
  const closeModalWeightProduct = () =>
    dispatch({ type: "CLOSE_MODAL_WEIGHT_PRODUCT" });
